...
def connect():
    ...
    if game_room in game_rooms and len(game_room) == 3:
        return (
            "Game room is full, please wait for one game room to end before trying again",
            400,
        )
    ...
    if client in game_rooms[game_room]["clients"]:
        return (
            "Client already exists in game room, please disconnect existing client",
            400,
        )
    ...
    if len(game_rooms[game_room]["clients"]) == 3:
        ...
        ready_data = {"GameState": "Ready"}

        ready_data_json = json.dumps(ready_data)

        if (
            not send_data_to_client(
                game_rooms[game_room]["clients"]["c"][0],
                "game_start_state",
                ready_data_json,
            )
            == "Success"
...
def add_safe_coordinates(game_room: str):
    ...
    data = {
        ...
        "Message": "Successfully received safe coordinates",
    }
    json_data = json.dumps(data)

    if (
        send_data_to_client(
            game_rooms[game_room]["clients"]["a"][0], "safe_coordinates", json_data
        )
        == "Success"
    ):
        ...
        if not send_data_to_client(
            game_rooms[game_room]["clients"]["b"][0], "game_start", message
        ):
...
async def send_data_to_client(ip, endpoint, data):
    host = ip
    port = (
        5000  # Make use of standardized port or make it customizable via command line
    )
...
def acknowledge_safe_coordinates(game_room):
    ...
    if client == "A" and game_rooms[game_room]["safe_coordinates"]:
        return (
            jsonify({"message": "Start game for Client B and Client C"}),
            200,
        )  # Sends a "Start game" message to Client B and Client C if the conditions are met.
    return "Safe coordinates not acknowledged or not Client A.", 400


@app.route(
    "/get_game_state/<game_room>", methods=["GET"]
)  # Retrieves the current game state for a specific game room and  links the URL /get_game_state/<game_room> to the function get_game_state.
...
def send_game_win_result(game_room):
    ...
    if not send_data_to_client(
        game_rooms[game_room]["clients"]["b"][0], "game_state", result
    ):
        return "Could not connect to client b", 500

    if not send_data_to_client(
        game_rooms[game_room]["clients"]["a"][0], "game_state", result
    ):
...
def send_game_loss_result(game_room):
    ...
    if not send_data_to_client(
        game_rooms[game_room]["clients"]["a"][0], "game_end_loss", result
    ):
        return "Could not connect to client a", 500

    if not send_data_to_client(
        game_rooms[game_room]["clients"]["c"][0], "game_end_loss", result
    ):
